...
import UIKit
import Firebase
class ViewController2: UIViewController,UITableViewDataSource, UITableViewDelegate,UIPopoverPresentationControllerDelegate
{
    let database = DataService.ds.GMBP_ZONE1 as Firebase!
   ...
   //   ------------------------------------------------------- more btn
    ...
    {
        self.performSegueWithIdentifier("showPopoverSegue2", sender: nil)
    }
    
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?)
    {
        if segue.identifier == "showPopoverSegue2"
        {
...
//        {
//            ViewController1.b2.removeFromSuperview()
//        }
//        
//    }
        ...
        self.database.observeSingleEventOfType(.Value, withBlock:
            {   snapshot in
                ViewController1.zone2Amout = snapshot.childrenCount
                 if var part3 = NSUserDefaults.standardUserDefaults().objectForKey("zone2Amount") as? UInt
                {
                    if(part3 > ViewController1.zone2Amout)
                    {
                        part3 = ViewController1.zone2Amout
                        print("PART3<VC1.ZONE1AMOUNT")
                    }
                    
                    let dif = String(ViewController1.zone2Amout - part3)
                    switch dif
                    {
                    case "0":
                        if message != ""
                        ...
                        break
                    case "1":
                        self.updateLabel.text = String(ViewController1.zone2Amout - part3) + " new post updated"
                        UIView.animateWithDuration(2)
                            { () -> Void in
                                self.updateLabel.alpha = 1
                                self.updateLabel.alpha = 0
                        ...
                        break
                    default:
                        self.updateLabel.text = String(ViewController1.zone2Amout - part3) + " new posts updated"
                        UIView.animateWithDuration(2)
                            { () -> Void in
                                self.updateLabel.alpha = 1
                                self.updateLabel.alpha = 0
                ...
                NSUserDefaults.standardUserDefaults().setObject(ViewController1.zone2Amout, forKey: "zone2Amount")
                ViewController1.b2.removeFromSuperview()
    ...
    func checkNet()
    {
        ...
        {
            titleButton.setTitle("BPI", forState: UIControlState.Normal)
            titleButton.setTitleColor(UIColor(red: 0.0, green: 0.4, blue: 0.0, alpha: 1.0) , forState: UIControlState.Normal)
        }
        else
        {
            titleButton.setTitle("BPI (Disconnected)", forState: UIControlState.Normal)
        ...
        {
            let post = posts[posts.count - 1 - indexPath.row]
            if let cell = tableView.dequeueReusableCellWithIdentifier("Cell2")  as? Cell2
            {
