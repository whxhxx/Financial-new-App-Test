...
import UIKit
import Firebase
class ViewController5: UIViewController,UITableViewDataSource, UITableViewDelegate,UIPopoverPresentationControllerDelegate
{
        let database = DataService.ds.GMBP_ZONE4 as Firebase!
    ...
    {
        self.performSegueWithIdentifier("showPopoverSegue5", sender: nil)
    }
    
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?)
    {
        if segue.identifier == "showPopoverSegue5"
        {
...
//        {
//            ViewController1.b5.removeFromSuperview()
//        }
//        
//    }
        ...
        self.database.observeSingleEventOfType(.Value, withBlock:
            {   snapshot in
                ViewController1.zone5Amout = snapshot.childrenCount
                 if var part3 = NSUserDefaults.standardUserDefaults().objectForKey("zone5Amount") as? UInt
                {
                    if(part3 > ViewController1.zone5Amout)
                    {
                        part3 = ViewController1.zone5Amout
                        print("PART3<VC1.ZONE1AMOUNT")
                    }
                    let dif = String(ViewController1.zone5Amout - part3)
                    switch dif
                    {
                    case "0":
                        if message != ""
                        ...
                        break
                    case "1":
                        self.updateLabel.text = String(ViewController1.zone5Amout - part3) + " new post updated"
                        UIView.animateWithDuration(2)
                            { () -> Void in
                                self.updateLabel.alpha = 1
                                self.updateLabel.alpha = 0
                        ...
                        break
                    default:
                        self.updateLabel.text = String(ViewController1.zone5Amout - part3) + " new posts updated"
                        UIView.animateWithDuration(2)
                            { () -> Void in
                                self.updateLabel.alpha = 1
                                self.updateLabel.alpha = 0
                ...
                NSUserDefaults.standardUserDefaults().setObject(ViewController1.zone5Amout, forKey: "zone5Amount")
                ViewController1.b5.removeFromSuperview()
    ...
    func checkNet()
    {
        ...
        {
             titleButton.setTitle("BPV", forState: UIControlState.Normal)
            titleButton.setTitleColor(UIColor(red: 0.0, green: 0.4, blue: 0.0, alpha: 1.0) , forState: UIControlState.Normal)
        }
        else
        {
             titleButton.setTitle("BPV (Disconnected)", forState: UIControlState.Normal)
        ...
        func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell
        {
         ...
         //   print(post.postDescription)
            ...
            if let cell = tableView.dequeueReusableCellWithIdentifier("Cell5")  as? Cell5
            {
